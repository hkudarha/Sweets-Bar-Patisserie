...
const GiftFinder = () => {
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-white/10 backdrop-blur-sm'>
      <div className="max-w-md mx-auto mt-10 bg-white rounded-2xl py-[1rem] px-[4rem] space-y-3">
        <h2 className=" text-center text-[#0e4d65]">Gift Finder</h2>
        <p className="text-center ">Find the Perfect Gift in Just <span className="underline">3 mins</span></p>
        ...
        <div ref={occasionRef}>
          ...
          >
            <span className='flex gap-2 items-center'><MdCelebration /> {selectedOccasion || 'Select Occasion'}</span>
            <svg className="w-4 h-4 text-gray-500" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
            </svg>
          </div>
          {showOccasionDropdown && (
            <ul className="mt-1 border rounded-lg max-h-40 overflow-y-auto bg-white shadow animate-fade-in z-10 absolute ">
